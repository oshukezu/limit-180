...
(function() {
  const Result = {
    renderEndGameResult(isCompleted, accuracy, starsEarned, avgTime, isPass, hasProfile) {
      ...
      // 渲染結算文字與數值
      document.getElementById('result-level').textContent = `M${this.gameState.currentMission} ${String(this.gameState.currentLevel).padStart(2, '0')}/20 - ${MISSION_CONFIGS[this.gameState.currentMission].desc}`;
