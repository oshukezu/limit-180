...
(function() {
  ...
  async function upsertPromoCode(code, coinsReward, expiresAt, maxTotalRedemptionsInput) {
    ...
    if (maxInputRaw && maxInputRaw !== 'all') {
      ...
      if (!Number.isInteger(parsed) || parsed <= 0) {
        throw new Error("總名額請輸入正整數或 all");
      }
    ...
    if (error) {
      console.error("[SupabaseService] upsertPromoCode 錯誤：", error.message);
      throw error;
    }
    return data;
  }
  ...
  async function listPromoCodes() {
    ...
    if (error) {
      console.error("[SupabaseService] listPromoCodes 錯誤：", error.message);
      throw error;
    }
    return data || [];
  }
  ...
  async function setPromoCodeActive(code, isActive) {
    ...
    if (error) {
      console.error("[SupabaseService] setPromoCodeActive 錯誤：", error.message);
      throw error;
    }
    return data;
  }
  ...
  async function deletePromoCode(code) {
    ...
    if (error) {
      console.error("[SupabaseService] deletePromoCode 錯誤：", error.message);
      throw error;
    }
    return true;
  }
  ...
  async function redeemPromoCode(code, gradeClass, seatNumber, nickname) {
    ...
    return {
      code: normalized,
      coinsReward: promo.coins_reward || 0,
